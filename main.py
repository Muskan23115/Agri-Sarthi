...
def build_context(crop: Optional[str], location: str) -> Dict:
...
async def webhook(request: Request):
    ...
    if content_type.startswith("application/json"):
        ...
        crop = None
        if any(k in user_query.lower() for k in ["wheat", "गेहूं", "gehun", "gehu"]):
            crop = "Wheat"
        elif any(k in user_query.lower() for k in ["mustard", "सरसों", "sarson"]):
            crop = "Mustard"

        data = build_context(crop, location)
